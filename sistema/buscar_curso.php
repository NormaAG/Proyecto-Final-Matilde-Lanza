...
<body>
	...
	<section id="container">
		...
		<a href="registro_curso.php" class="btn_new" style="font-size:15pt; text-decoration:none;"><i class="icon-office"></i> Crear Nuevo Curso </a>
		<?php }?>
		...
		<?php 
			//Paginador
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM curso WHERE (grado LIKE '%$busqueda%'
                                                                                    					) 
																										 AND estatus = 1");
			$result_register = mysqli_fetch_array($sql_registe);
			$total_registro = $result_register['total_registro'];

			$por_pagina = 5; //numero de usuario a mostrar por pagina
			...
			$query = mysqli_query($conection,"SELECT * FROM curso WHERE (grado LIKE '%$busqueda%' OR seccion LIKE '%$busqueda%') AND estatus = 1 ORDER BY idcurso ASC LIMIT $desde,$por_pagina ");
			mysqli_close($conection);
