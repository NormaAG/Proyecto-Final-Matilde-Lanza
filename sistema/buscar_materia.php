...
<body>
	...
	<section id="container">
		...
		<a href="registro_materia.php" class="btn_new" style="font-size:15pt; text-decoration:none;"><i class="icon-profile"></i> Crear Nueva Materia </a>
		<?php } ?>
		...
		<?php 
			//Paginador
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM materia WHERE (materia LIKE '%$busqueda%' ) AND estatus = 1");
			...
			$query = mysqli_query($conection,"SELECT * FROM materia WHERE (materia LIKE '%$busqueda%') AND estatus = 1 ORDER BY idmateria ASC LIMIT $desde,$por_pagina ");
			mysqli_close($conection);
